 ...
 *
 */
class PMCProcessor extends AbstractProcessor {

    XmlParser parser

    PMCProcessor(FileSystemIterator files, Inserter solr, int nThreads) {
        super(files, solr, nThreads)
        parser = Factory.createXmlParser()
    }

    public LappsDocument process(File file) {
        return process(file.newInputStream())
    }

    public LappsDocument process(InputStream stream) {
